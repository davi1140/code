...
# Basically, analyzing the entire situation rather than just one sample size should help increase point total. (Better version of detective)
# Reminder: For the history array, "tell truth" = 0, "stay silent" = 1

def strategy(history, memory):
    Schedule = ["stay silent","tell truth","stay silent"]
    ...
    test = gLength
    while(test > 1): # Testing whether the cycle has reached a multiple of 3  or not
        test = test % 3
        if(test > 0)
            Switches = 0 
        elif(test == 1):
